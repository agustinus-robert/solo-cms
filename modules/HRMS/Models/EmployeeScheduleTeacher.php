...
class EmployeeScheduleTeacher extends Model
{
    use Searchable;
     ...
     */
    protected $table = 'empl_schedules_teachers';
        ...
        foreach ($this->getAttribute('dates') as $date => $shifts) {
            ...
            foreach ($shifts as $i => $shift) {
                ...
                if ($withNull ? $shift : array_filter($shift)) {
                    ...
                    $customTimes = [];
                    if (!empty($shift)) {
                        $customTimes[$workshift->value] = [
                            'in'  => $shift[0] ?? null,
                            'out' => $shift[1] ?? null,
                        ];
                    }

                    $adjustedTime = $workshift->adjustTime($customTimes);
                    $inTime = Carbon::parse("{$adjustedTime['in']}");
                    $outTime = Carbon::parse("{$adjustedTime['out']}");

                    $in = isset($scanlogs[$date]) ? $scanlogs[$date]
                        ->filter(function ($log) use ($workshift, $inTime, $outTime, $previousOutTime) {
                            $createdAt = Carbon::instance($log->created_at->toDateTime())->setTimezone('Asia/Jakarta');

                            if ($previousOutTime === null) {
                                return $createdAt->gte($inTime) || $createdAt->lte($outTime);
                            } else {
                                return $createdAt->gte($previousOutTime) && $createdAt->lte($outTime);
                            }
                        })
                        ->values()
                    ...
                    $entries[$date][] = (object) [
                        ...
                        'in' => $in !== null ? Carbon::parse($in) : null,
                        'out' => $out,
                        'modifier' => isset($shift[2]) ? $shift[2] : null,
