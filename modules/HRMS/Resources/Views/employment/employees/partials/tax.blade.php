<div class="card border-0">
    <div class="card-body p-3">                
        <form action="{{ route('hrms::employment.tax-action', ['employee' => $employee->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="form-check form-switch d-flex align-items-center">
                    <input class="form-check-input" type="checkbox" name="is_tax_active" id="tax_status" value="1" 
                        {{ $employee->getMeta('tax_status') ? 'checked' : '' }} 
                        onchange="this.form.submit()" 
                        style="width: 40px; height: 20px; cursor: pointer;">
                    
                    <label class="form-check-label ms-2 fw-bold {{ $employee->getMeta('tax_active') ? 'text-success' : 'text-secondary' }}" 
                        for="tax_status" style="font-size: 0.7rem; letter-spacing: 0.5px;">
                        {{ $employee->getMeta('tax_status') ? 'aktif' : 'tidak aktif' }}
                    </label>
                </div>
            </div>
        </form>
