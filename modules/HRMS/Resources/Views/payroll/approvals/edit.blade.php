...
@section('content')
    <div class="d-flex align-items-center mb-4">
        <a class="text-decoration-none" href="{{ request('next', route('hrms::payroll.approvals.index')) }}"><i class="mdi mdi-arrow-left-circle-outline mdi-36px"></i></a>
        <div class="ms-4">
            <h2 class="mb-1">Persetujuan gaji</h2>
            <div class="text-secondary">Formulir dibawah untuk divalidasi oleh Manajer Keuangan</div>
        </div>
    </div>
    <div class="card border-0">
        <div class="card-body">
            <i class="mdi mdi-file-plus-outline"></i> Formulir persetujuan gaji
        </div>
        <div class="card-body border-top border-light">
            <form class="form-block form-confirm" action="{{ route('hrms::payroll.approvals.update', ['salary' => $salary->id, 'next' => request('next', route('hrms::payroll.approvals.index'))]) }}" method="POST"> @csrf @method('PUT')
                ...
                <div class="row">
                    <div class="offset-lg-3 offset-xl-2 col-lg-9 col-xl-10">
                        ...
                        <div class="text-danger mb-3">
                            <strong>Perhatian!</strong> Anda hanya dapat menyetujui permohonan ini 1x, jika Anda tidak menyetujui, harap abaikan formulir ini, terima kasih.
                        </div>
                        <div class="form-check mb-3">
                            <input class="form-check-input" id="agreement" type="checkbox" name="approved" value="1" required>
                            <label class="form-check-label" for="agreement">Dengan ini saya selaku Manajer Keuangan menyetujui permohonan gaji <strong>{{ $salary->name }}</strong> karyawan atas nama <strong>{{ $employee->user->name }}</strong></label>
                        </div>
                        <div>
                            <button class="btn btn-soft-danger"><i class="mdi mdi-check"></i> Simpan</button>
                            <a class="btn btn-ghost-light text-dark" href="{{ request('next', route('hrms::payroll.approvals.index')) }}"><i class="mdi mdi-arrow-left"></i> Kembali</a>
                        </div>
                    </div>
                </div>
            </form>
