                                        ...
                                        <div class="col-xl-{{ 12 / count($workshifts) }}">
                                            <div class="d-flex">
                                                ...
                                                <input class="form-check-input"
                                                        ...
                                                        data-in="{{ $shift === \Modules\HRMS\Enums\WorkShiftEnum::NORMAL
                                                            ? ($shift->defaultTime()['in'][1] ?? $shift->defaultTime()['in'][0])
                                                            : $shift->defaultTime()['in'][0] }}"

                                                        data-out="{{ $shift === \Modules\HRMS\Enums\WorkShiftEnum::NORMAL
                                                            ? ($shift->defaultTime()['out'][1] ?? $shift->defaultTime()['out'][0])
                                                ...
                                                <div class="input-group">
                                                    <input type="time" class="form-control in time-{{ $date }}-{{ $i }}" name="dates[{{ $date }}][{{ $i }}][]" value="{{ $moment ? '' : $worktime_default[date('w', strtotime($date))][$i][0] ?? '' }}">
                                                    <div class="input-group-text">s.d.</div>
                                                    <input type="time" class="form-control out time-{{ $date }}-{{ $i }}" name="dates[{{ $date }}][{{ $i }}][]" value="{{ $moment ? '' : $worktime_default[date('w', strtotime($date))][$i][1] ?? '' }}">
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            @foreach ($contracts->groupBy('position.position.department.name') as $department => $_contracts)
                                <optgroup label="{{ $department ?: 'Lainnya' }}">
                                    @forelse($_contracts->sortBy('employee.user.name') as $contract)
                                        <option value="{{ $contract->empl_id }}" @selected(in_array($contract->empl_id, old('empl_ids', [])))>{{ $contract->employee->user->name }} &mdash; {{ $contract->position->position->name }} ({{ $contract->empl_id }})</option>
                                    @empty
