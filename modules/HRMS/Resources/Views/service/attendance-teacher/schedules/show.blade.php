                                    ...
                                    @foreach ($workshifts as $i => $shift)
                                        <div class="col-xl-{{ 12 / count($workshifts) }}">
                                            <div class="input-group">
                                                <input type="time" class="form-control time-{{ $date }}-{{ $i }}" name="dates[{{ $date }}][{{ $i }}][]" value="{{ isset($schedule->dates[$date][$i][0]) ? date('H:i', strtotime($schedule->dates[$date][$i][0])) : '' }}">
                                                <div class="input-group-text">s.d.</div>
                                                <input type="time" class="form-control time-{{ $date }}-{{ $i }}" name="dates[{{ $date }}][{{ $i }}][]" value="{{ isset($schedule->dates[$date][$i][1]) ? date('H:i', strtotime($schedule->dates[$date][$i][1])) : '' }}">
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
