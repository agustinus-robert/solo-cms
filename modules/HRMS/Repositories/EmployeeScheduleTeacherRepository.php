...
trait EmployeeScheduleTeacherRepository
{
    ...
    {
        $schedule = new EmployeeScheduleTeacher(Arr::only($data, ['empl_id', 'start_at', 'end_at', 'dates', 'workdays_count']));
        if ($schedule->save()) {
    ...
    public function updateEmployeeSchedule(EmployeeScheduleTeacher $schedule, array $data)
    {
        ...
        if ($schedule->save()) {
            Auth::user()->log('memperbarui jadwal kerja karyawan ' . $schedule->employee->user->name . ' <strong>[ID: ' . $schedule->id . ']</strong>', EmployeeScheduleTeacher::class, $schedule->id);
            return $schedule;
        }
        return false;
    }
