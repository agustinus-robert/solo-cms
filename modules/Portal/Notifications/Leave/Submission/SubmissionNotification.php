...
class SubmissionNotification extends Notification implements ShouldQueue
{
     ...
     */
    public function __construct(EmployeeLeave $leave, ?EmployeePosition $position)
    {
        $this->leave = $leave;
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ...
            ->line($this->leave->employee->user->name . ' mengajukan izin ' . $this->leave->category->name . '' . ($this->position ? ' yang telah disetujui oleh ' . $this->position->position->name . ' ' : '') . ' dengan keperluan ' . ($this->leave->description ?: 'istirahat') . ', klik tombol di bawah untuk lihat detailnya.')
    ...
    {
        return [
            'message' => $this->leave->employee->user->name . ' mengajukan izin' . ($this->position ? ' yang telah disetujui oleh ' . $this->position->position->name . ' ' : '') . ', cek sekarang!',
