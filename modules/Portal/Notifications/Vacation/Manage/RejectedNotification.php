...
class RejectedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        return $mail->subject('Maaf ' . $this->vacation->quota->employee->user->name . '! Pengajuan ' . $this->label . ' kamu belum disetujui')
            ...
            ->line('Pengajuan ' . $this->label . ' kamu sebanyak ' . collect($this->vacation->dates)->count() . ' hari untuk keperluan ' . ($this->vacation->description ?: 'istirahat') . ' belum disetujui oleh ' . $this->approvable->userable->position->level->label() . ', klik tombol di bawah untuk lihat detailnya.')
    ...
    {
        return [
            'message' => 'Maaf! Pengajuan ' . $this->label . ' kamu sebanyak ' . collect($this->vacation->dates)->count() . ' hari untuk keperluan ' . ($this->vacation->description ?: 'istirahat') . ' belum disetujui oleh ' . $this->approvable->userable->position->level->label() . '.',
