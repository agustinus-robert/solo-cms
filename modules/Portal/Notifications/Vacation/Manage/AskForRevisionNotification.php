...
class AskForRevisionNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        return $mail->subject('Hai ' . $this->vacation->quota->employee->user->name . '! Silakan revisi pengajuan cuti/libur hari raya kamu')
            ...
            ->line('Pengajuan cuti/libur hari raya kamu sebanyak ' . collect($this->vacation->dates)->count() . ' hari untuk keperluan ' . ($this->vacation->description ?: 'istirahat') . ' telah mendapat pesan untuk revisi dari ' . $this->approvable->userable->position->level->label() . ', klik tombol di bawah untuk lihat detailnya.')
    ...
    {
        return [
            'message' => 'Pengajuan cuti/libur hari raya kamu sebanyak ' . collect($this->vacation->dates)->count() . ' hari untuk keperluan ' . ($this->vacation->description ?: 'istirahat') . ' telah mendapat pesan untuk revisi dari ' . $this->approvable->userable->position->level->label() . '.',
