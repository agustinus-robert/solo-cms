...
class SubmissionNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->line($this->vacation->quota->employee->user->name . ' mengajukan ' . $this->label . '' . ($this->position ? ' yang telah disetujui oleh ' . $this->position->position->name . ' ' : '') . ' sebanyak ' . collect($this->vacation->dates)->count() . ' hari untuk keperluan ' . ($this->vacation->description ?: 'istirahat') . ', klik tombol di bawah untuk lihat detailnya.')
