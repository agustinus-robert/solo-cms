...
namespace Modules\Portal\Notifications\Additional\Manage;
...
class ApprovedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('Selamat! Pengajuan lembur kegiatan tambahan kamu disetujui')
            ->greeting('Selamat! Pengajuan lembur kegiatan tambahan kamu disetujui')
            ->line('Pengajuan lembur ' . $this->additional->name . ' telah disetujui oleh ' . $this->approvable->userable->position->level->label() . ', klik tombol di bawah untuk lihat detailnya.')
    ...
    {
        return [
            'message' => 'Selamat! Pengajuan lembur ' . $this->additional->name . ' telah disetujui oleh ' . $this->approvable->userable->position->level->label() . '.',
