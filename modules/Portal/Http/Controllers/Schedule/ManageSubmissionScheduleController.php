...
use Modules\Portal\Notifications\Schedule\AllocateSubmission\Submission\AllocativeSubmissionNotification;
use Modules\Portal\Notifications\Schedule\Submission\AllocateSubmission\ApprovedNotification;
use Modules\Portal\Notifications\Schedule\Submission\AllocateSubmission\RejectedNotification;
use Modules\Portal\Notifications\Schedule\Submission\AllocateSubmission\RevisedNotification;
use Carbon\Carbon;
use Modules\Portal\Excel\RowSchedule;
...
class ManageSubmissionScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $submissions = EmployeeScheduleSubmission::search($request->get('search'))
        ...
        return view('portal::schedules.manage.submission.index', compact('submissions', 'submission_count', 'start_at', 'end_at'));
    }
    ...
    public function show(EmployeeScheduleSubmission $submission, Request $request)
    {
        ...
        $calendarData = EmployeeScheduleSubmission::whereMonth('start_at', $month)
        ...
        return view('portal::schedules.manage.submission.show', compact('month', 'workshifts', 'databaseResult', 'calendarData', 'type', 'scheduleSubmission', 'employee', 'results', 'submissionId'));
    }
    ...
    public function SubmissionUpdate(SubmissionUpdateRequest $request, CompanyApprovable $approvable)
    {
        ...
        if ($approvable->cancelable) {
            ...
            if ($request->input('result') == ApprovableResultEnum::APPROVE->value) {
                $approveAt = EmployeeScheduleSubmission::find($request->id);
                $approveAt->update(['approved_at' => now()]);

                $existingSch = EmployeeSchedule::where(['empl_id' => $approveAt->empl_id, 'start_at' => $approveAt->start_at, 'end_at' => $approveAt->end_at])->first();
