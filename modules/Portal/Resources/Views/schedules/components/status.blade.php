...
@else
    @if ($schedule->dates->max('d') < date('Y-m-d'))
        <div class="badge bg-soft-secondary text-dark fw-normal"><i class="mdi mdi-check"></i> Selesai</div>
    @else
        @if ($schedule->hasApprovables() && !$schedule->hasAllApprovableResultIn('APPROVE'))
            @if ($schedule->approvableTypeIs('approvable'))
                @if ($schedule->hasAllApprovableResultIn('APPROVE'))
                    <div class="badge bg-soft-success text-success fw-normal"><i class="mdi mdi-exit-to-app"></i> Pengajuan disetujui</div>
                @elseif ($schedule->hasAnyApprovableResultIn('REJECT'))
