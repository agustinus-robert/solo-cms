...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            <h2 class="mb-1">Kelola Pengajuan Jadwal</h2>
            <div class="text-muted">Kamu dapat mengunggu/menolak lembur karyawan!</div>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-8">
            <div class="card border-0">
                <div class="card-body">
                    <i class="mdi mdi-calendar-multiselect"></i> Daftar pengajuan jadwal kerja
                </div>
                @if (request('pending'))
                            ...
                            @forelse($submissions as $allocate)
                                <tr>
                                    ...
                                    <td nowrap class="py-1 text-end">
                                        <a href="{{route('portal::schedule.submission.show', ['submission' => $allocate->id])}}" class="btn btn-soft-primary px-2 py-1" data-bs-toggle="tooltip" data-bs-title="Kelola data"><i class="mdi mdi-progress-clock"></i></a>
                                        <form class="d-inline form-block form-confirm" action="{{ route('portal::schedule.submission.update', $allocate->id) }}" method="POST" id="update-form-{{ $allocate->id }}"> @csrf @method('PUT')
        ...
        <div class="col-xl-4">
            <div class="card border-0">
                ...
                <div class="list-group list-group-flush border-top">
                    <a class="list-group-item list-group-item-action text-danger" href="{{ route('portal::schedule.submission.index', ['pending' => !request('pending')]) }}"><i class="mdi mdi-progress-clock"></i> {{ request('pending') == 1 ? 'Tampilkan semua pengajuan' : 'Hanya tampilkan pengajuan yang tertunda' }}</a>
                </div>
            </div>
            <a class="btn btn-outline-primary w-100 d-flex text-primary mb-4 rounded bg-white py-3 text-start" style="border-style: dashed;" href="{{ route('portal::schedule.manages.collective') }}">
            ...
            <div class="card border-0">
                <div class="card-body">
                    <form class="form-block" action="{{ route('portal::schedule.manage.submission.import') }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-10">
                ...
                <div class="card-body border-top">
                    <form class="form-block" action="{{ route('portal::schedule.submission.index') }}" method="get">
                        <div class="mb-3">
