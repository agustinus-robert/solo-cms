                ...
                <div class="row gy-2 @if (!$loop->last) mb-4 @endif">
                    ...
                    <div class="col-md-12">
                        @if ($approvable->userable->is($employee->position) && !$scheduleSubmission->trashed())
                            <form class="form-block" action="{{ route('portal::schedule.submissionappr.update', ['approvable' => $approvable->id, 'next' => request('next', route('portal::schedule.submission.index'))]) }}" method="post"> @csrf @method('PUT')
                                <input type="hidden" name="id" value="{{$submissionId}}" />
                                <div class="mb-3">
                                    <select class="form-select @error('result') is-invalid @enderror" name="result">
                                        @foreach ($results as $result)
                                            @unless(($approvable->cancelable && in_array($result, Modules\HRMS\Models\EmployeeScheduleSubmission::$cancelable_disable_result)) || in_array($result, Modules\HRMS\Models\EmployeeScheduleSubmission::$approvable_disable_result ?? []))
