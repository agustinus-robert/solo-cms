...
@section('content')
    ...
    <form class="form-block" action="{{ route('portal::schedule.manage.store', ['next' => request('next')]) }}" method="POST"> @csrf
        <div class="card mb-4 border-0">
            <div class="card-body">
                <input type="hidden" name="contract_id" value="{{ $contract->id ?? null }}" readonly>
                                        ...
                                        <div class="col-xl-{{ 12 / count($workshifts) }}">
                                            <div class="d-flex">
                                                <div class="form-check py-2">
                                                    <input class="form-check-input" type="checkbox" value="" id="" data-in="{{ $shift->defaultTimes()['in'][0] }}" data-out="{{ $shift->defaultTimes()['out'][0] }}" onclick="renderMe(event)" {{ $class }} />
                                                </div>
                                                <div class="input-group">
