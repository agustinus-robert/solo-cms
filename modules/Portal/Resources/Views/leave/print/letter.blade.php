    ...
    <section>
        ...
        <p class="bold">
            ...
            <table width="100%">
                ...
                'Waktu pengajuan' => Carbon::parse($leave['created_at'])->isoFormat('LLLL'),
                'Kategori izin' => $leave->category->name,
                'Tanggal izin' => collect($leave->dates)->map(fn($date) => strftime('%d %B %Y', strtotime($date['d'])) . (isset($date['t_s']) ? ' pukul ' . $date['t_s'] : '') . (isset($date['t_e']) ? ' s.d. ' . $date['t_e'] : ''))->join(', '),
