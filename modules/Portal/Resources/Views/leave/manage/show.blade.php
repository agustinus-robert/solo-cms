...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            ...
            <div class="text-muted">Kamu dapat menyetujui/menolak/merevisi pengajuan izin yang diajukan karyawan!</div>
        </div>
    </div>
    @if ($leave->trashed())
    ...
    <div class="row">
        <div class="col-xl-8">
                    ...
                    <div class="card-body border-top">
                        @foreach ($leave->approvables->sortBy('level') as $approvable)
                            ...
                            <div class="row gy-2 @if (!$loop->last) mb-4 @endif">
                                ...
                                <div class="col-md-6">
                                    @if ($approvable->userable->position_id === $employee->position->position_id && !$leave->trashed())
                                        <form class="form-block" action="{{ route('portal::leave.manage.update', ['approvable' => $approvable->id, 'next' => request('next', route('portal::leave.manage.index'))]) }}" method="post"> @csrf @method('PUT')
                                            <div class="mb-3">
            ...
            <div class="col-xl-4">
                ...
                'Humas' => $leave->employee->position->position->parents->firstWhere('level.value', 2)?->employees->first()->user->name,
            ]) as $label => $value)
