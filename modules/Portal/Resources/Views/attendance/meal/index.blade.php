...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            <h2 class="mb-1">Presensi makan</h2>
            <div class="text-muted">Yuk! Lakukan check-in sebelum makan!</div>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-4">
            <div class="card border-0">
                ...
                <div class="card-body py-md-5 py-4 text-center">
                    ...
                    <div class="tg-steps-presence-button my-4">
                        @if (($current_schedule && !$vacations->pluck('d')->contains(date('Y-m-d'))) || ($current_schedule && !$holidays->pluck('d')->contains(date('Y-m-d'))))
                            @if (request('month', date('Y-m')) == date('Y-m'))
                                <form class="form-block" action="{{ route('portal::attendance.meal.index') }}" method="post"> @csrf
                                    <input class="d-block form-scan form-control mx-auto mb-2 text-center" type="hidden" name="latlong" value="">
            ...
            @if ($location->value == Modules\Core\Enums\WorkLocationEnum::WFA->value)
                <div class="card tg-steps-presence-move border-0">
                    <div class="card-body d-flex align-items-center">
                        <img src="{{ asset('img/manypixels/Motorcycle_Flatline.svg') }}" style="max-width: 200px;" alt="">
                        <div class="flex-grow-1">
                            <h5 class="mb-1">Mau ke kantor?</h5>
                            <p class="text-muted">Presensi kamu otomatis kerekam setelah pindah ke kantor!</p>
                            @if ($current_schedule && !$vacations->pluck('d')->contains(date('Y-m-d')))
                                <form class="form-block" action="{{ route('portal::attendance.presence.index', ['next' => route('portal::attendance.presence.index', ['type' => Modules\Core\Enums\WorkLocationEnum::WFO->name])]) }}" method="post"> @csrf
                                    <input class="d-block form-scan" type="hidden" name="latlong" value="">
                                    <input type="hidden" name="location" value="{{ Modules\Core\Enums\WorkLocationEnum::WFO->value }}">
                                    <button class="btn btn-soft-danger btn-sm rounded px-3" type="submit"><i class="mdi mdi-map-marker-outline"></i> Pindah lokasi & scan</button>
                                    <div id="geolocation-notice" class="d-none text-danger text-center">Biar bisa presensi, <br> kamu wajib ngaktifin lokasi browser kamu!</div>
                                </form>
                            @else
                                <button class="btn btn-soft-secondary text-muted btn-sm rounded px-3" disabled><i class="mdi mdi-map-marker-outline"></i> Pindah lokasi & scan</button>
                            @endif
                        </div>
                    </div>
                </div>
                                        ...
                                        <td class="{{ $_date == date('Y-m-d') ? 'bg-soft-secondary' : '' }}" style="height: 100px; min-height: 100px; min-width: 120px;">
                                            @if ((($week == 1 && $dayindex >= $_month->dayOfWeek) || $week > 1) && ($week != $totalWeekOfMonth || $day <= $_month->daysInMonth))
                                                ...
                                                @if ($vacations->pluck('d')->contains($_date))
                                                    ...
                                                    @php($cl = $fr[0] != null ? '--bs-secondary' : '--bs-primary')
                                                ...
                                                @endif
                                                @if ($holidays->pluck('d')->contains($_date))
                                                    <div class="position-relative float-end">
                                                        <div class="position-absolute text-center text-white" style="transform: rotate(45deg); top: -1px; right: -23px; border-bottom: 18px solid var(--bs-danger); font-size: .75rem; border-left: 18px solid transparent; border-right: 18px solid transparent; height: 0;">
                                                            {{ 'Libur' }}
                                                        </div>
                                                    </div>
                                                @endif
