            ...
            <div class="container-fluid">
                ...
                <div class="d-flex align-items-center mb-4">
                    ...
                    <div class="ms-4">
                        ...
                        <div class="text-muted">Berikut adalah informasi detail pengajuan insentif kegiatan kamu!</div>
                    </div>
                </div>
                @if ($outwork->trashed())
                    ...
                    <div class="col-xl-4">
                        ...
                        @unless ($outwork->hasAnyApprovableResultIn('REJECT') || !$outwork->hasApprovables() || $outwork->trashed())
                            @if ($outwork->hasAllApprovableResultIn('PENDING') || $outwork->hasAnyApprovableResultIn('REVISION') || !$outwork->hasApprovables())
                                <form class="form-block form-confirm" action="{{ route('portal::outwork.submission.destroy', ['outwork' => $outwork->id]) }}" method="post"> @csrf @method('delete')
                                    <button class="btn btn-outline-danger w-100 text-danger d-flex align-items-center bg-white py-3 text-start">
                                        ...
                                        <div>Batalkan pengajuan <br> <small class="text-muted">Hapus data pengajuan {{ $outwork->hasApprovables() ? 'sebelum disetujui oleh atasan' : '' }}</small></div>
                                    </button>
                                </form>
                            @endif
                        @endunless
