...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            <h2 class="mb-1">Hari libur</h2>
            <div class="text-muted">Berikut adalah informasi detail pengajuan hari libur karyawan!</div>
        </div>
    </div>
    @if ($holiday->trashed())
        ...
        <div class="col-xl-8">
            <div class="card border-0">
                <div class="card-body d-flex align-items-center justify-content-between">
                    ...
                    @if (!$holiday->trashed())
                        <a class="btn btn-soft-success btn-sm rounded px-2 py-1" href="{{ route('portal::holiday.print', ['holiday' => $holiday->id]) }}" target="_blank"><i class="mdi mdi-printer-outline"></i> <span class="d-none d-sm-inline">Cetak dokumen (.pdf)</span></a>
                    @endif
                </div>
                <div class="card-body border-top">
                    <div class="mb-4">
                        <div class="small text-muted">Tanggal pengajuan</div>
                        <div class="fw-bold"> {{ $holiday->created_at->formatLocalized('%A, %d %B %Y') }}</div>
                    </div>
                    <div class="mb-4">
                        <div class="small text-muted mb-1">Tanggal hari libur yang diajukan</div>
                        <div>
            ...
            <div class="col-xl-4">
                ...
                'Nama karyawan' => $holiday->employee->user->name,
                'NIP' => $holiday->employee->kd ?: '-',
                'Jabatan' => $holiday->employee->position->position->name ?? '-',
                'Departemen' => $holiday->employee->position->position->department->name ?? '-',
                'Manajer' => $holiday->employee->position->position->parents->firstWhere('level.value', 4)?->employees->first()->user->name,
            ]) as $label => $value)
                ...
                @unless (!$holiday->hasApprovables() || $holiday->trashed())
                    @if ($holiday->hasAllApprovableResultIn('PENDING') || $holiday->hasAnyApprovableResultIn('REVISION') || $holiday->hasAnyApprovableResultIn('REJECT') || !$holiday->hasApprovables())
                        <form class="form-block form-confirm" action="{{ route('portal::holiday.submission.destroy', ['holiday' => $holiday->id]) }}" method="post"> @csrf @method('delete')
                            <button class="btn btn-outline-danger w-100 text-danger d-flex align-items-center bg-white py-3 text-start">
                                ...
                                <div>Batalkan pengajuan <br> <small class="text-muted">Hapus data pengajuan {{ $holiday->hasApprovables() ? 'sebelum disetujui oleh atasan' : '' }}</small></div>
                            </button>
                        </form>
                    @endif
                @endunless
