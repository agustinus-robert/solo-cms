...
<div class="card mb-4 border-0">
    <div class="card-body">
        <form class="form-block" action="{{ route('core::company.positions.update', ['position' => $position->id, 'next' => request('next')]) }}" method="POST"> @csrf @method('PUT')
                            ...
                            <select class="form-select @error('default_applied_role') is-invalid @enderror" name="default_applied_role">
                                ...
                                @foreach($roles as $_role)
                                    <option value="{{ $_role->id }}"
                                        @selected(old('default_applied_role', $position->getMeta('default_applied_role')) == $_role->id)>
                                        {{ $_role->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="col-xxl-7 col-xl-6">
                    ...
                    <div class="mb-3 row">
                        <div class="col-lg-8 offset-lg-4 offset-xl-3">
                            ...
                            <a class="btn btn-ghost-light text-dark" href="{{ route('core::company.positions.store', ['next' => request('next')]) }}"><i class="mdi mdi-arrow-left"></i> Kembali</a>
                        </div>
                    </div>
                </div>
            </div>
