...
<div class="row justify-content-center">
    <div class="col-xxl-8 col-xl-10">
        <div class="d-flex align-items-center mb-4">
            <a class="text-decoration-none" href="{{ request('next', route('core::company.services.leave-categories.index')) }}"><i class="mdi mdi-arrow-left-circle-outline mdi-36px"></i></a>
            <div class="ms-4">
                <h2 class="mb-1">Ubah kategori izin</h2>
                <div class="text-secondary">Silakan isi formulir di bawah ini untuk mengubah kategori izin</div>
            </div>
        </div>
        <div class="card mb-4 border-0">
            <div class="card-body">
                <form class="form-block" action="{{ route('core::company.services.leave-categories.update', ['category' => $category->id, 'next' => request('next')]) }}" method="POST"> @csrf @method('PUT')
                        ...
                        <div class="col-xl-8 col-xxl-6">
                            <select class="form-select @error('parent_id') is-invalid @enderror" name="parent_id">
                                @forelse($categories as $_category)
                                    ...
                                    @endif
                                    <option value="{{ $_category->id }}" @if(old('parent_id', $category->parent_id) == $_category->id) selected @endif>{{ $_category->name }}</option>
                                @empty
                        ...
                        <div class="col-md-4">
                            @foreach (['start_to_end' => 'Menampilkan jam mulai izin dan jam izin akhir', 'start_only' => 'Hanya menampilkan jam mulai izin'] as $v => $description)
                                <div class="form-check d-flex align-items-center">
                                    <input class="form-check-input only_one" type="checkbox" id="time_input_{{ $v }}" value="{{ $v }}" name="time_input" @if(old('time_input', $category->meta?->time_input ?? -1) == $v) checked @endif>
                                    <label class="form-check-label ms-2" for="time_input_{{ $v }}"><code>{{ $v }}</code> <br> <small class="text-muted">{{ $description }}</small></label>
                                </div>
                            @endforeach
                            @error('time_input')
                                <small class="text-danger d-block"> {{ $message }} </small>
                    ...
                    <div class="mb-3 row">
                        <div class="col-lg-8 offset-lg-4 offset-xl-3">
                            ...
                            <a class="btn btn-ghost-light text-dark" href="{{ route('core::company.services.leave-categories.store', ['next' => request('next')]) }}"><i class="mdi mdi-arrow-left"></i> Kembali</a>
                        </div>
                    </div>
                </form>
            </div>
