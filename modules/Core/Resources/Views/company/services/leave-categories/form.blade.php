    ...
    <div class="col-xxl-8 col-xl-10">
        <div class="card mb-4 border-0">
            <div class="card-header bg-gradient-dark text-white">
                <h6 class="text-white">{{ $isEdit ? 'Ubah' : 'Tambah' }} Kategori Izin</h6>
            </div>
            ...
            <div class="card-body shadow-sm">
                <form
                    method="POST"
                    action="{{ $isEdit
                        ? route('core::company.services.leave-categories.update', ['category' => $category->id, 'next' => request('next')])
                        : route('core::company.services.leave-categories.store', ['next' => request('next')])
                    }}"
                >
                    @csrf
                    @if($isEdit)
                            ...
                            @foreach([
                                ...
                                <div class="form-check mb-3 d-grid" style="grid-template-columns: 20px 1fr;">
                                    <input
                                        ...
                                        id="time_input_{{ $v }}"
                                        value="{{ $v }}"
                                        @checked(old('time_input', $isEdit ? data_get($category->meta, 'time_input') : null) === $v)
                                    >
                    ...
                    <x-input-group>
                        <x-col size="12" offset="3">
                            ...
                            href="{{ request('next', route('core::company.services.leave-categories.index')) }}">
                                Kembali
                            </a>
                        </x-col>
                    </x-input-group>
