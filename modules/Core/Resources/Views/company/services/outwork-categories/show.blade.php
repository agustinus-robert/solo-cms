    ...
    <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-10">
            <div class="d-flex align-items-center mb-4">
                ...
                <div class="ms-4">
                    <h2 class="mb-1">Ubah kategori kegiatan lainnya</h2>
                    <div class="text-secondary">Silakan isi formulir di bawah ini untuk mengubah kategori kegiatan lainnya</div>
                </div>
            </div>
            <div class="card mb-4 border-0">
                <div class="card-body">
                    <form class="form-block" action="{{ route('core::company.services.outwork-categories.update', ['category' => $category->id, 'next' => request('next')]) }}" method="POST"> @csrf @method('put')
                        ...
                        <div class="row required mb-3">
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <input type="text" class="form-control @error('description') is-invalid @enderror" name="description" value="{{ old('description', $category->description) }}" required />
                                @error('description')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-xl-8 col-xxl-8">
                                <div class="input-group">
                                    ...
                                    <input placeholder="Di luar jam kerja" type="number" class="form-control @error('price') is-invalid @enderror" name="price" value="{{ old('price', $category->price) }}" />
                                    ...
                                    <input placeholder="Di jam kerja" type="number" class="form-control @error('in_working_hours_price') is-invalid @enderror" name="in_working_hours_price" value="{{ old('in_working_hours_price', $category->meta?->in_working_hours_price ?? '') }}" />
                                </div>
                                @error('price')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            ...
                            <div class="col-md-4">
                                @foreach (['prepareable' => 'Persiapan kegiatan', 'fixed' => 'Tarif flat'] as $v => $description)
                                    <div class="form-check">
                                        <input class="form-check-input only_one" type="checkbox" id="meta{{ $v }}" value="1" name="meta[{{ $v }}]" @if (old('meta.$v', $category->meta?->{$v} ?? '') == 1) checked @endif>
