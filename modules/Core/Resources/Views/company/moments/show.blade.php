    ...
    <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-10">
            <div class="d-flex align-items-center mb-4">
                ...
                <div class="ms-4">
                    <h2 class="mb-1">Ubah hari libur</h2>
                    <div class="text-secondary">Silakan isi formulir di bawah ini untuk mengubah hari libur</div>
                </div>
            </div>
            <div class="card mb-4 border-0">
                <div class="card-body">
                    <form class="form-block" action="{{ route('core::company.moments.update', ['moment' => $moment->id, 'next' => request('next')]) }}" method="POST"> @csrf @method('PUT')
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <select class="form-select @error('type') is-invalid @enderror" name="type" required>
                                    @forelse($types as $_type)
                                        ...
                                        @endif
                                        <option value="{{ $_type->value }}" @selected(old('type', $moment->type->value) == $_type->value)>{{ $_type->label() }}</option>
                                    @empty
                        ...
                        <div class="row required mb-3">
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $moment->name) }}" required />
                                @error('name')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-xl-6 col-xxl-4">
                                <input type="date" class="form-control @error('date') is-invalid @enderror" name="date" value="{{ old('date', $moment->date) }}" />
                                @error('date')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-xl-6 col-xxl-4 pt-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_holiday" value="1" name="is_holiday" @checked(old('is_holiday', $moment->is_holiday) == 1)>
                                    <label class="form-check-label" for="is_holiday"><strong><span id="is_holiday-text">
                                                @if (old('is_holiday', $moment->is_holiday) == 1)
