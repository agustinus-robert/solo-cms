...
class Menu extends Component
{
    ...
    public function mount(Request $req)
    {
        ...
        if (!empty($id) && is_string($id)) {
            ...
            $this->size_image = (empty(json_decode($data_to_cookie->image_code)->size_image) ? '' :  json_decode($data_to_cookie->image_code)->size_image);
            $this->orientation_image = (empty(json_decode($data_to_cookie->image_code)->orientation_image) ? '' :  json_decode($data_to_cookie->image_code)->orientation_image);

            $this->add = (empty($data_to_cookie->add) ? 1 :  (string) $data_to_cookie->add);
            $this->edit = (empty($data_to_cookie->edit) ? 1 : (string) $data_to_cookie->edit);
            $this->album = (empty($data_to_cookie->album) ? 1 : (string) $data_to_cookie->album);
            $this->delete = (empty($data_to_cookie->delete) ? 1 : (string) $data_to_cookie->delete);
            $this->video = (empty($data_to_cookie->video) ? 1 : (string) $data_to_cookie->video);
            ...
            if (isset($_COOKIE['_x_meta_keyword'])) {
                if (!isset($this->meta_keyword[$this->language])) {
                    $this->meta_keyword = $data_to_cookie->meta_keyword;;
                } else {
                    $this->meta_keyword[$this->language] = json_decode($data_to_cookie->meta_keyword, true)[$this->language];
                }
            } else {
                $this->meta_keyword = json_decode($data_to_cookie->meta_keyword, JSON_UNESCAPED_SLASHES);
            }
        ...
        $this->menu_category = CmsMenuCategory::where(['deleted_at' => null])->get()->toArray();
        $this->list_menu = CmsMenu::where(['deleted_at' => null])->get();
    ...
    public function change_add($event)
    {
        $this->add = $event;
    }

    public function change_edit($event)
    {
        $this->edit = $event;
    }

    public function change_album($event)
    {
        $this->album = $event;
    }

    public function change_video($event)
    {
        $this->video = $event;
    }

    public function change_delete($event)
    {
        $this->delete = $event;
    }
    ...
    public function submitForm()
    {
        if (empty($this->title)) {
            ...
            $arr_save['meta_keyword'] = json_encode($this->meta_keyword);
            ...
            $arr_save['add'] = $this->add;
            $arr_save['edit'] = $this->edit;
            $arr_save['delete'] = $this->delete;
            $arr_save['album'] = $this->album;
            ...
            $arr_save['image_code'] = json_encode([
                ...
                'orientation_image' => $this->orientation_image
            ]);
            ...
            if ($this->data_id) {
                ...
                if (!empty($this->relasi_menu)) {
                    CmsMenuRelated::updateOrCreate(
                        ['from_menu' => $this->data_id],
                        ['from_menu' => $this->data_id, 'with_menu' => $this->relasi_menu]
                    );
                }
            } else {
                ...
                $menu = CmsMenu::create($arr_save);
                // event(new MenuCreated('Menu telah ditambahkan silahkan klik <b>Sync</b>, untuk melihat pembaharuan fitur'));
                if (!empty($this->relasi_menu)) {
                    //DB::table('menu')->insert();
                    CmsMenuRelated::create(['from_menu' => $menu->id, 'with_menu' => $this->relasi_menu]);
                }
            }

            //$this->clearForm($_COOKIE);
            $url = route('cms::builder.menu.index');
    ...
    {
        return view('cms::livewire.builder.menu');
    }
}
