            ...
            <section>
                <div class="card border-0">
                    ...
                    <div class="card-body border-top border-light">
                        <form class="form-block row gy-2 gx-2" action="{{ route('cms::system.users.index') }}" method="get">
                            <input name="trash" type="hidden" value="{{ request('trash') }}">
                            <div class="flex-grow-1 col-auto">
                                <input class="form-control" name="search" placeholder="Cari nama atau username ..." value="{{ request('search') }}" />
                            </div>
                            <div class="col-auto">
                                <a class="btn btn-light" href="{{ route('cms::system.users.index', request()->only('trashed', 'closed')) }}"><i class="mdi mdi-refresh"></i> <span class="d-sm-none">Reset</span></a>
                            </div>
                            <div class="col-auto">
                                ...
                                @forelse($users as $user)
                                    <tr @if ($user->trashed()) class="table-light text-muted" @endif>
                                        ...
                                        <td class="fw-bold" nowrap>
                                            ...
                                            @else
                                                <a class="text-dark" href="{{ route('cms::system.users.show', ['user' => $user->id, 'page' => 'profile', 'next' => url()->current()]) }}">{{ $user->name }}</a>
                                            @endif
                                        </td>
                                        <td>{{ $user->username }}</td>
                                        ...
                                        <td class="py-2 text-end" nowrap>
                                            @if ($user->isnot(Auth::user()))
                                                @if ($user->trashed())
                                                    @can('restore', $user)
                                                        <form class="form-block form-confirm d-inline" action="{{ route('cms::system.users.restore', ['user' => $user->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('put')
                                                            <button class="btn btn-soft-info rounded px-2 py-1" data-bs-toggle="tooltip" title="Pulihkan"><i class="mdi mdi-refresh"></i></button>
                                                        </form>
                                                        <form class="form-block form-confirm d-inline" action="{{ route('cms::system.users.kill', ['user' => $user->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('delete')
                                                ...
                                                @else
                                                    @can('show', $user)
                                                        <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('cms::system.users.show', ['user' => $user->id, 'page' => 'profile', 'next' => url()->full()]) }}" method="post" data-bs-toggle="tooltip" title="Lihat detail"><i class="mdi mdi-eye-outline"></i></a>
                                                    @endcan
                                                    @can('destroy', $user)
                                                        <form class="form-block form-confirm d-inline" action="{{ route('cms::system.users.destroy', ['user' => $user->id, 'next' => url()->full()]) }}" method="post"> @csrf @method('delete')
                                                    ...
                                                    @can('update', $user)
                                                        <div class="dropstart d-inline">
                                                            ...
                                                            <ul class="dropdown-menu border-0 shadow">
                                                                ...
                                                                @endcan
                                                                <li>
                                                                    <form class="dropdown-item form-block form-confirm" action="{{ route('cms::system.users.repass', ['user' => $user->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('put')
        ...
        <div class="col-md-4">
            ...
            @can('store', Modules\Account\Models\User::class)
                <div class="card border-0">
                    ...
                    <div class="card-body border-top">
                        <form class="form-block" action="{{ route('cms::system.users.store', ['next' => url()->full()]) }}" method="post"> @csrf
                            <div class="mb-3">
            ...
            <div class="card border-0">
                ...
                <div class="list-group list-group-flush border-top border-light">
                    <a class="list-group-item list-group-item-action text-danger" href="{{ route('cms::system.users.index', ['trash' => !request('trash')]) }}"><i class="mdi mdi-trash-can-outline"></i> Lihat pengguna yang {{ request('trash') ? 'tidak' : '' }} dihapus</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            document.getElementById('modal-cross-login').addEventListener('show.bs.modal', (e) => {
                ...
                document.getElementById('modal-cross-login-form').setAttribute('action', `{{ route('cms::system.users.index') }}/${user.id}/login`);
