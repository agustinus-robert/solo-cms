...
@section('title', env('APP_NAME') . ' Pengembalian Barang')

@section('navtitle', env('APP_NAME') . ' Pengembalian Barang')

@section('content')

    @if (str_contains(url()->full(), 'create') || str_contains(url()->full(), 'edit'))
        @livewire('poz::transaction.return-goods', ['action' => $action])
    @else
        @php
            $arr = [
                'global' => false,
                'column' => $column,
                'ajax' => [
                    'url' => route('poz::transaction.return.datatables'),
