...
@section('title', env('APP_NAME') . ' Purchase')

@section('navtitle', env('APP_NAME') . ' Purchase')

@section('content')


    @if (str_contains(url()->full(), 'create') || str_contains(url()->full(), 'edit'))
        @livewire('poz::transaction.purchase', ['action' => $action])
    @else
        @php
            $arr = [
                'global' => false,
                'column' => $column,
                'ajax' => [
                    'url' => route('poz::transaction.purchase.datatables'),
