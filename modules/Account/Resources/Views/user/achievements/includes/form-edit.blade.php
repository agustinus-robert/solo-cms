<div class="form-group required row">
	...
	<div class="col-md-6 col-lg-7">
		<input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $achievement->name) }}" required autofocus>
	...
	<div class="col-md-6">
		<select name="type_id" class="form-control @error('type_id') is-invalid @enderror" required>
			...
			@foreach (\App\Models\References\AchievementType::all() as $v)
				<option value="{{ $v->id }}" @if((old('type_id', $achievement->type_id) == $v->id)) selected @endif>{{ $v->name }}</option>
			@endforeach	
		</select>
		@error('type_id')
	...
	<div class="col-md-6">
		<select name="territory_id" class="form-control @error('territory_id') is-invalid @enderror" required>
			...
			@foreach (\App\Models\References\Territory::all() as $v)
				<option value="{{ $v->id }}" @if((old('territory_id', $achievement->territory_id) == $v->id)) selected @endif>{{ $v->name }}</option>
			@endforeach	
		</select>
		@error('territory_id')
			<span class="invalid-feedback"> {{ $message }} </span>
...
<div class="form-group required row">
	...
	<div class="col-md-4">
		<input type="number" class="form-control @error('year') is-invalid @enderror" name="year" value="{{ old('year', $achievement->year) }}" required>
		@error('year')
			<span class="invalid-feedback"> {{ $message }} </span>
		@enderror
	</div>
	...
	<div class="col-md-6">
		<select name="num_id" class="form-control @error('num_id') is-invalid @enderror" required>
			...
			@foreach (\App\Models\References\AchievementNum::all() as $v)
				<option value="{{ $v->id }}" @if((old('num_id', $achievement->num_id) == $v->id)) selected @endif>{{ $v->name }}</option>
			@endforeach	
		</select>
		@error('num_id')
			<span class="invalid-feedback"> {{ $message }} </span>
...
<div class="form-group row">
	...
	<div class="col-md-6 col-lg-7">
		<div class="pb-4 d-flex justify-content-center">
			<img id="upload-preview" class="w-100 rounded border @error('file') border-danger @enderror" src="{{ $achievement->file_path }}" alt="">
		</div>
