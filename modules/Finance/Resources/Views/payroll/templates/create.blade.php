...
@section('content')
    ...
    <div class="card mb-4 border-0">
        <div class="card-body">
            <form class="form-block" action="{{ route('finance::payroll.templates.store', ['employee' => $employee->id, 'next' => request('next')]) }}" method="POST"> @csrf
                <div class="row mb-3">
                                ...
                                @foreach ($templates as $template)
                                    <label class="card-body border-secondary d-flex align-items-center @if (!$loop->last) border-bottom @endif py-2">
                                        ...
                                        <div>
                                            ...
                                            <div class="small text-muted">Dibuat {{ $template->created_at->diffForHumans() }} dengan {{ $template->components ? count($template->components) : 0 }} komponen</div>
                                        </div>
                                    </label>
                                @endforeach
                                                ...
                                                <td style="min-width: 120px;">
                                                    ...
                                                    <select class="form-select" data-name="component_id" onchange="renderUnitComponent(event.currentTarget)">
                                                        ...
                                                        @foreach ($category->components as $component)
                                                            <option value="{{ $component->id }}" data-currency="{{ $component->currency }}" data-unit="{{ $component->unit?->label() }}" data-default="{{ $component->meta->default ?? null }}" data-description="{{ $component->meta->description ?? null }}" data-disabled="{{ $component->unit?->disabledState() }}">{{ $component->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td style="min-width: 120px;">
                                                    <div class="input-group">
                                                        <input type="text" oninput="validatedRupiah(this)" data-name="amount" class="form-control money-currency" disabled required>
