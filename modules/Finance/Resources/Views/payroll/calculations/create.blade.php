                                                            ...
                                                            <tr class="calc-row @if ($loop->first) calc-row-template @endif">
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <div class="input-group flex-nowrap">
                                                                        ...
                                                                        <input @if ($category !== 'Rekapitulasi')
                                                                             oninput="validatedRupiah(this)"
                                                                        @endif 
                                                                        data-no-rupiah="{{ $category === 'Rekapitulasi' ? 1 : 0 }}"
                                                                        type="text" class="form-control text-end" data-name="amount" value="0" min="0" onkeyup="renderRealAmountValue(event.currentTarget)" required>
...
@push('scripts')
    <script>
        ...
        const renderAmountValue = () => {
            [...document.querySelectorAll('[data-name="component_id"] option:checked')].forEach((el, i) => {
                ...
                amount.value = ['jam'].includes(el.dataset.unit.toLowerCase()) 
                    ? parseFloat(defaultAmount).toFixed(2) // Langsung ambil defaultAmount tanpa dikali multiplier lagi jika multipliernya sudah 100
                    : Math.round(defaultAmount * (parseFloat(el.dataset.multiplier) || 1));                amount.dataset.operateSymbol = el.dataset.operateSymbol;
        ...
        const renderRealAmountValue = (el) => {
            ...
            // 1. Logika pengambilan angka
            if (el.dataset.noRupiah == "1") {
                // Jika Rekapitulasi: Titik adalah desimal. JANGAN DIHAPUS.
                // Ganti koma ke titik (antisipasi input standar Indo)
                cleanValue = parseFloat(rawValue.replace(/,/g, '.')) || 0;
            } else {
                // Jika Rupiah: Titik adalah ribuan. HARUS DIHAPUS.
                cleanValue = parseFloat(rawValue.replace(/\./g, '')) || 0;
            }

            // 2. Logika Operasi (+ atau -)
            // Ambil simbol dari dataset yang sudah di-render oleh renderAmountValue
        ...
        //             [...tbody.querySelectorAll('[data-name="amount"]')].map(el =>
        //                 parseFloat((el.dataset.realAmount || '0').replace(/\./g, ''))
        //             ).reduce((result, x) => result + x, 0)
        //         );
        //         tbody.querySelector('.calc-row-subtotal-input').value = subtotal.toLocaleString('id-ID');
        ...
        const sumSubTotal = () => {
            [...document.querySelectorAll('.calc-tbody')].forEach(tbody => {
                ...
                let subtotal = [...tbody.querySelectorAll('[data-name="amount"]')]
                    .map(el => parseFloat(el.dataset.realAmount || 0)) // Langsung ambil angka murni
                    .reduce((result, x) => result + x, 0);
        ...
        const sumSlipTotal = () => {
            [...document.querySelectorAll('.calc-table')].forEach(table => {
                let sliptotal = [...table.querySelectorAll('[data-name="amount"]')]
                    .map(el => parseFloat(el.dataset.realAmount || 0)) // Gunakan angka murni dari dataset
                    .reduce((result, x) => result + x, 0);

                table.querySelector('.calc-slip-total-input').classList.toggle('text-danger', sliptotal < 0);
                // Tampilkan dengan format ID, tapi untuk Rekapitulasi tetap akan muncul benar
                table.querySelector('.calc-slip-total-input').value = sliptotal.toLocaleString('id-ID');
        ...
        const sumTHP = () => {
            let thptotal = [...document.querySelectorAll('.calc-slip-total-input')]
                .map(el => {
                    // Kita balikkan format lokalisasi ke angka murni
                    let val = el.value.toString().replace(/\./g, '').replace(/,/g, '.');
                    return parseFloat(val || 0);
                })
                .reduce((thp, x) => thp + x, 0);

            // Gunakan Math.round hanya untuk THP akhir jika memang harus bulat
