...
@section('navtitle', 'Laporan lembur')
                        ...
                        <table class="mb-0 table align-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Lembur terbayar</th>
                                    <th class="text-center">Disetujui/Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($employees as $employee)
                                    @php
                                        $overtimes = $employee->overtimes
                                            ->filter(fn($overtimes) => $overtimes->hasAllApprovableResultIn('APPROVE'))
                                            ->unique('id')
                                            ->flatMap(
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            {{ Str::money($total_time) }} jam
                                        </td>
                                        <td class="text-center">
                                            {{ Str::money($employee->overtimes->sum('paid_amount'), 0) }}
                                        </td>
                                        <td class="text-center">{{ $employee->overtimes->filter(fn($o) => $o->hasAllApprovableResultIn('APPROVE'))->count() }}/{{ $employee->overtimes->count() }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="8">
        ...
        <div class="col-md-4">
            <div class="card border-0">
                ...
                <div class="card-body border-top">
                    <form class="form-block" action="{{ route('finance::report.overtimes.index') }}" method="get">
                        <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <a class="btn btn-light" href="{{ route('finance::report.overtimes.index', ['start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d')]) }}"><i class="mdi mdi-refresh"></i> Reset</a>
                        </div>
                    </form>
                </div>
            </div>
            <div class="card border-0">
                ...
                <div class="list-group list-group-flush border-top">
                    <button class="list-group-item list-group-item-action @if (!$employees->count()) disabled @endif py-3" onclick="exportExcel()"><i class="mdi mdi-file-excel-outline me-3"></i> Unduh laporan lembur</button>
                </div>
                <div class="card-body border-top">
...
@push('scripts')
    ...
    @include('finance::report.overtimes.components.excel-script')
    <script>
