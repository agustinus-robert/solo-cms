...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            ...
            <div class="text-secondary">Anda dapat menambahkan pinjaman karyawan dengan mengisi formulir di bawah</div>
        </div>
    </div>
    <div class="row">
        <div class="col-xl-8">
            <div class="card mb-4 border-0">
                <div class="card-body">
                    <form class="form-block" action="{{ route('finance::service.loans.store', ['next' => request('next')]) }}" method="POST" enctype="multipart/form-data"> @csrf
                                    ...
                                    <div class="overflow-auto rounded" style="max-height: 300px;">
                                        @forelse($categories as $category)
                                            <label class="card-body border-secondary d-flex align-items-center @if (!$loop->last) border-bottom @endif py-2">
                                                <input class="form-check-input me-3" type="radio" value="{{ $category->id }}" name="ctg_id" data-has-interest="{{ $category->interest_id }}" data-interest="{{ $category->meta?->interest ?? '' }}" data-max="{{ $category->meta?->tenor ?? '' }}" data-file="{{ $category->meta?->file ?? '' }}" data-multiply="{{ $category->meta?->multiplied_by_tenor ?? '' }}" required>
                                                <div>
                                                    <div class="fw-bold mb-0">{{ $category->name }}</div>
                                                    <div class="small text-muted">{{ $category->description ?? 'Tidak ada deskripsi' }} </div>
                        ...
                        <div class="row required mb-3">
                            ...
                            <div class="col-xl-8 col-xxl-4">
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount') }}" oninput="calculateSimulationTable()" required />
                                </div>
                                @error('amount')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-xl-4 col-xxl-2">
                                <div class="input-group">
                                    ...
                                    <input class="form-check-input me-1 mt-0" type="hidden" id="multiply" value="1" name="multiply" readonly>
                        ...
                        <div class="row required mb-3">
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <div class="input-group">
                                    <input type="number" class="form-control @error('tenor') is-invalid @enderror" name="tenor" value="{{ old('tenor', 1) }}" onkeyup="validateTenor(event);calculateStartDate(event);calculateSimulationTable();" required />
                        ...
                        <div class="row required mb-3">
                            <label class="col-lg-4 col-xl-3 col-form-label">Tgl penagihan selanjutnya</label>
                            <div class="col-xl-8 col-xxl-6">
                                <input type="date" class="form-control @error('start_at') is-invalid @enderror" name="start_at" value="{{ old('start_at') }}" onchange="calculateSimulationTable()" required />
                                @error('start_at')
                                    <small class="text-danger d-block"> {{ $errors->first('start_at') }} </small>
                                @enderror
                            </div>
                        </div>
                        <div class="row form-file d-none mb-3">
...
@push('scripts')
    ...
    <script>
        ...
        const calculateStartDate = (e = false) => {
            if (submission_at = document.querySelector('[name="submission_at"]').value) {
                if (operator = document.querySelector('[name="tenor_by"] :checked').dataset.operator) {
                    document.querySelector('[name="start_at"]').value = moment(submission_at).add(1, operator).format('Y-MM-DD')
                }
            }
        }

        const toggleHasInterestInput = (e) => {
            document.querySelector('[name="has_interest"]').checked = false;
            document.querySelector('[name="has_interest"]').toggleAttribute('disabled', !e.currentTarget.querySelector(':checked').dataset.hasInterest)
            toggleInterestInput();
        }
        ...
        const toggleInterestInput = (e) => {
            ...
            document.querySelector('[name="has_interest"]').value = e.target.dataset.hasInterest ? e.target.dataset.hasInterest : '';
            document.querySelector('[name="interest_value"]').value = e.target.dataset.hasInterest ? e.target.dataset.interest : '';

            e.target.dataset.max > 0 ?
                document.querySelector('[name="tenor"]').setAttribute('max', e.target.dataset.max) :
                document.querySelector('[name="tenor"]').removeAttribute('max');

            document.querySelector('[name="multiply"]').value = e.target.dataset.multiply ? e.target.dataset.multiply : '';
        ...
        const calculateSimulationTable = debounce(() => {
            ...
            let multiplied = parseFloat(document.querySelector('[name="multiply"]').value);
            ...
            if (amount && tenor && operator && start_at) {
                for (let i = 1; i <= tenor; i++) {
                    ...
                    tr += `<td>${(parseInt(multiplied ? amount : amount/tenor)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</td>`;
