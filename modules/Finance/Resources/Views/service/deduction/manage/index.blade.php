                            ...
                            @forelse($employees as $employee)
                                <tr @if ($employee->trashed()) class="text-muted" @endif>
                                    ...
                                    <td class="text-center">
                                        @if (count($employee->dataRecapitulations) > 0)
                                            {{ $employee->dataRecapitulations->pluck('result.total')->first() }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td nowrap class="py-1 text-end">
                                        @unless ($employee->trashed())
                                            ...
                                            @if (count($employee->dataRecapitulations) > 0)
                                                <a class="btn btn-soft-danger btn-sm rounded px-2 py-1" data-bs-toggle="tooltip" title="Edit" href="{{ route('finance::service.deduction.manage.show', ['deduction' => $employee->id, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'next' => url()->full()]) }}"><i class="mdi mdi-pencil-outline"></i></a>
                                                <form class="form-block form-confirm d-inline" action="{{ route('finance::service.deduction.manage.destroy', ['deduction' => $employee->id, 'next' => url()->full()]) }}" method="POST"> @csrf @method('delete')
                                            ...
                                            @else
                                                <a class="btn btn-soft-info btn-sm rounded px-2 py-1" data-bs-toggle="tooltip" title="Rekap baru" href="{{ route('finance::service.deduction.manage.create', ['deduction' => $employee->id, 'start_at' => $start_at, 'end_at' => $end_at, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'next' => url()->full()]) }}"><i class="mdi mdi-plus-circle-outline"></i></a>
                                            @endif
                                        @endunless
                                    </td>
                                </tr>
