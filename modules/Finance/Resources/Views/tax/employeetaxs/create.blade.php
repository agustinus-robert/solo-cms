...
@section('title', 'Tambah NPWP | ')

@section('navtitle', 'Tambah NPWP')
        ...
        <div class="col-xxl-8 col-xl-10">
            ...
            <div class="card mb-4 border-0">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div>
                        <i class="mdi mdi-plus"></i> Tambah NPWP
                    </div>
                </div>
                <div class="card-body">
                    <form class="form-block" enctype="multipart/form-data" action="{{ route('finance::tax.employeetaxs.store', ['next' => request('next')]) }}" method="POST"> @csrf
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <select type="text" class="form-select @error('user_id') is-invalid @enderror" name="user_id" value="{{ old('user_id') }}" required>
                                    ...
                                    @foreach ($employees as $label => $employee)
                                        <optgroup label="{{ $label ?: 'Lainnya' }}">
                                            @foreach ($employee as $empl)
                                                <option value="{{ $empl->user->id }}" @selected(old('user_id'))>{{ $empl->user->name }} ({{ $empl->position->position->name }})</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-8 col-xl-9">
                                <textarea name="tax_address" type="text" class="form-control">{{ old('tax_address') }}</textarea>
                            </div>
                        </div>
                        <div class="row mb-3">
