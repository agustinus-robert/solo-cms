                            ...
                            @forelse($employees as $employee)
                                <tr @class(['table-active text-muted' => is_null($employee->contract)])>
                                    ...
                                    <td class="text-center">
                                        {{ $employee->overtimes_count }} /
                                        @if (count($employee->dataRecapitulations))
                                            {{ $employee->dataRecapitulations->pluck('result.overtimes')->flatten(1)->count() }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-end">
                                        @if (count($employee->dataRecapitulations))
                                            <a class="btn btn-soft-warning rounded px-2 py-1" href="{{ route('finance::summary.overtimes.create', ['employee' => $employee->id, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'edit' => $employee->dataRecapitulations->first()->id, 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Ubah"><i class="mdi mdi-pencil-outline"></i></a>
                                            <form class="form-block form-confirm d-inline" action="{{ route('finance::summary.overtimes.destroy', ['employee' => $employee->id, 'overtimes' => $employee->dataRecapitulations->implode('id', ',')]) }}" method="post"> @csrf @method('delete')
                                                <button class="btn btn-soft-danger rounded px-2 py-1" href="{{ route('finance::summary.overtimes.destroy', ['employee' => $employee->id, 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Hapus"><i class="mdi mdi-trash-can-outline"></i></button>
                                            </form>
                                        @else
                                            <a class="btn btn-soft-primary rounded px-2 py-1" href="{{ route('finance::summary.overtimes.create', ['employee' => $employee->id, 'start_at' => $start_at->format('Y-m-d'), 'end_at' => $end_at->format('Y-m-d'), 'next' => url()->full()]) }}" data-bs-toggle="tooltip" title="Rekap baru"><i class="mdi mdi-plus-circle-outline"></i></a>
                                        @endif
                                    </td>
                                </tr>
                            @empty
        ...
        <div class="col-xl-4">
            <div class="card border-0">
                ...
                <div class="card-body border-top">
                    <form class="form-block" action="{{ route('finance::summary.overtimes.index') }}" method="get">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <a class="btn btn-light" href="{{ route('finance::summary.overtimes.index') }}"><i class="mdi mdi-refresh"></i> Reset</a>
                        </div>
                    </form>
                </div>
            </div>
