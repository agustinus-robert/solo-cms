                ...
                <div class="card-body">
                    <form class="form-block" enctype="multipart/form-data" action="{{ route('finance::service.deduction.manage.store', ['next' => request('next', route('finance::service.deduction.manage.index'))]) }}" method="POST"> @csrf
                        <div class="row required mb-3">
                            ...
                            <div class="col-md-6">
                                <select name="empl" id="empl" class="form-select @error('empl') is-invalid @enderror" onchange="reloadCartId(event.currentTarget)" required>
                                    <option value=""> </option>
                                    @foreach ($employees as $empl)
                                        <option value="{{ $empl->id }}" data-route="{{ route('finance::service.deduction.manage.create', [...request()->only('empl_id', 'next'), 'empl_id' => request('empl', $empl->id)]) }}" @selected(old('empl', request('empl_id')) == $empl->id)>{{ $empl->user->name }}</option>
                                    @endforeach
                                </select>
                                @error('empl')
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                @foreach ($categories as $type)
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="type" id="type-{{ $type->value }}" value="{{ $type->value }}" required @checked(old('type', $type->value)) />
