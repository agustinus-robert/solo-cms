...
@section('title', 'Rekapitulasi Gaji ke-13 | ')
@section('navtitle', 'Rekapitulasi Gaji ke-13')

@php
    $profile = array_filter([
        'Nama karyawan' => $employee->user->name,
        'NIP' => $employee->kd ?: '-',
        'Bergabung' => $employee->joined_at?->isoFormat('LL') ?? '-',
...
@section('content')
    <div class="d-flex align-items-center mb-4">
        <a class="text-decoration-none" href="{{ request('next', route('finance::summary.postyear.index')) }}"><i class="mdi mdi-arrow-left-circle-outline mdi-36px"></i></a>
        <div class="ms-4">
            <h2 class="mb-1">Hitung rekapitulasi Gaji ke-13</h2>
            <div class="text-secondary">Menampilkan data penghitungan Gaji ke-13.</div>
        </div>
    </div>
    <div class="row">
        <div class="col-xxl-4">
        ...
        <div class="col-xxl-8 order-xxl-first">
            <div class="card border-0">
                ...
                <form class="form-confirm form-block" action="{{ route('finance::summary.postyear.store', ['employee' => $employee->id, 'next' => request('next', route('finance::summary.postyear.index'))]) }}" method="post"> @csrf
                    <div class="card-body">
                        <div class="input-group">
                            <div class="input-group-text">Periode cut off</div>
                            <input class="form-control" type="text" value="{{ $start_at->format('Y-m-d') }}" readonly disabled>
                            <input class="form-control" type="date" value="{{ $end_at->format('Y-m-d') }}" readonly disabled>
                        </div>
                        <input class="d-none" name="start_at" type="date" value="{{ $start_at->format('Y-m-d') }}">
                        <input class="d-none" name="end_at" type="date" value="{{ $end_at->format('Y-m-d') }}">
                    </div>
                        ...
                        <table class="table-hover table-bordered table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Jumlah</th>
                                    <th>
                                        <button type="button" class="btn btn-secondary rounded-circle btn-delete-row px-2 py-1" onclick="addRow(event.target)"><i class="mdi mdi-trash-can-outline"></i></button>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($periods as $k => $period)
                                    ...
                                    @foreach ($tmps as $_k => $tmp)
                                        <tr class="period-tr">
                                            <td>
                                                <input class="form-control" data-name="el.m" data-id="{{ $tmp->id ?? '' }}" data-item="{{ $tmp ? $tmp->items->pluck('id') : [0] }}" type="month" onkeyup="renderAmountSubTotal()" value="{{ $month }}">
                                            </td>
                                            <td class="cmp_id">
                                                <select class="form-select" data-name="el.template" id="" onchange="renderTemplateItems(event.target)">
                                                    @foreach ($salaryTemplates as $template)
                                                        <option value="{{ $template->id }}" data-components="{{ $template->components }}" @selected(str_contains($template->name, 'Gaji ke-13'))>{{ $template->name }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{-- value="{{ count($periods) }}" --}}
                                        <input class="form-control" name="result[division]" type="number" value="12" onkeyup="renderAmountTotal()">
                                    </td>
                                    <td></td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input class="form-control" name="result[total]" type="number" readonly>
                                    </td>
                                    <td></td>
                                </tr>
                            </tbody>
...
@push('scripts')
    <script>
        const benefit = @json(\Modules\HRMS\Enums\DataRecapitulationTypeEnum::G13);
        ...
        const renderByData = (currentEl = false) => {
            [...document.querySelectorAll('[data-name="el.m"]')].forEach((el, idx) => {
                ...
                [...arr].forEach((x, y) => {
                    document.querySelector(`[name="result[el][${idx}][components][${y}][id]"]`).value = x;
                });
            });
        };
        ...
        const renderSubTotal = () => {
            Object.keys(templates).forEach(key => {
                let sub = Math.round([...document.querySelectorAll('.group-' + key)].map(el => parseFloat(el.value || 0)).reduce((s, x) => s + x, 0));
