...
use Modules\Finance\Http\Requests\Payment\Additional\StoreRequest;
...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('finance::payment.additional.index', [
            'start_at'    => $start_at,
            'end_at'      => $end_at,
            'departments' => CompanyDepartment::visible()->with('positions')->get(),
            'employees'   => Employee::with(['user', 'contract.position.position', 'dataRecapitulations' => fn($recap) => $recap->whereType(DataRecapitulationTypeEnum::ADDITIONAL_PAYMENT)->whereStrictPeriodIn($start_at, $end_at)])
                ->withCount(['additional_overtimes' => fn($addt) => $addt->whereMethod(PaymentMethodEnum::DIRECT)->whereBetween('paidable_at', [$start_at, $end_at])])
                ->whereHas('contract')
                ->whereHas('additional_overtimes', fn($addt) => $addt->whereMethod(PaymentMethodEnum::DIRECT)->whereBetween('paidable_at', [$start_at, $end_at]))
    ...
    public function create(Request $request)
    {
        ...
        $data = $employee->additional_overtimes()->whereMethod(PaymentMethodEnum::DIRECT)->whereBetween('paidable_at', [$start_at, $end_at])->get()->each(function ($addt) use ($employee, $schedules) {
                ...
                if ($param) {
                    switch ($param->first()['id']) {
                        case AdditionalTypeEnum::ADDITIONAL:
                            $meal = isset($date['t_m']) ? $employee->{$param->first()['meal']['value']}() : 0;
                        ...
                        case AdditionalTypeEnum::VENUE:
                            $meal  = $inside > $outside ? $param->first()['meal']['value'][0] : $param->first()['meal']['value'][1];
                            $load  = $param->first()['load']['value'];
        ...
        return view('finance::payment.additional.create', [
            'employee'    => $employee,
            'start_at'    => $start_at,
            'end_at'      => $end_at,
    ...
    public function store(StoreRequest $request)
    {
        ...
        if ($current = EmployeeDataRecapitulation::whereType(DataRecapitulationTypeEnum::ADDITIONAL_PAYMENT)->find($request->get('edit'))) {
        ...
        $recap = $employee->dataRecapitulations()->updateOrCreate(
            [
                'type' => DataRecapitulationTypeEnum::ADDITIONAL_PAYMENT,
    ...
    public function destroy(Employee $employee, Request $request)
    {
        ...
        $employee->dataRecapitulations()->whereType(DataRecapitulationTypeEnum::ADDITIONAL_PAYMENT)->whereIn('id', $additionals)->get()->each(
